...
object SysColumnConfigFormProps {
    ...
    const val tableConfig = "tableConfig"

    /**
     * 获取所有字段名列表（按默认顺序）
     */
    fun getAllFields(): List<String> {
        return listOf(columnKey, columnComment, kmpType, sortOrder, showFilter, showSort, routeKey, tableConfig)
    }
}
...
fun SysColumnConfigFormOriginal(
    ...
    val defaultRenderMap = linkedMapOf<String, @Composable () -> Unit>(
        ...
        SysColumnConfigFormProps.tableConfig to {
            var dataList by remember { mutableStateOf<List<SysTableConfigIso>>(emptyList()) }

            LaunchedEffect(Unit) {
                try {
                    val provider = Iso2DataProvider.isoToDataProvider[SysTableConfigIso::class]
                    dataList = provider?.invoke("") as? List<SysTableConfigIso> ?: emptyList()
                } catch (e: Exception) {
                    println("加载 tableConfig 数据失败: ${e.message}")
                    dataList = emptyList()
                }
            }

            AddGenericSingleSelector(
                value = state.value.tableConfig,
                onValueChange = { state.value = state.value.copy(tableConfig = it) },
                placeholder = "tableConfig",
                dataProvider = { dataList },
                getId = { it.id ?: 0L },
                getLabel = { it.routeKey ?: "" },
...
class SysColumnConfigFormDsl(
    ...
    fun tableConfig(
        ...
        when {
            hidden -> {
                hiddenFields.add("tableConfig")
                renderMap.remove("tableConfig")
            }
            render != null -> {
                hiddenFields.remove("tableConfig")
                renderMap["tableConfig"] = { render(state) }
            }
            else -> {
                hiddenFields.remove("tableConfig")
                renderMap.remove("tableConfig")
        ...
        // 处理排序
        order?.let { orderValue ->
            updateFieldOrder("tableConfig", orderValue)
        }
    }
