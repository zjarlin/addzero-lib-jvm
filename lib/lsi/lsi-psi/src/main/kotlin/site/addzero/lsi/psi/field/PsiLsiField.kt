...
class PsiLsiField(private val psiField: PsiField) : LsiField {
  ...
  override val isEnum: Boolean
    get() {
      // 步骤 1：获取字段的 PsiType（类型）
      val fieldType = psiField.type ?: return false

      // 步骤 2：将 PsiType 转换为对应的 PsiClass（类/枚举）
      val fieldClass = PsiTypesUtil.getPsiClass(fieldType)

      // 步骤 3：判断 PsiClass 是否为枚举类
      val bool = fieldClass?.isEnum == true
      val bool1 = psiField is PsiEnumConstant
      return bool || bool1
    }
