 ...
 * 支持自定义显示名称和扫描器
 * 
 * @param displayName 设置面板的显示名称
 * @param configScanner 配置扫描器函数，用于扫描并注册配置类
 */
open class BaseConfigurableTreeUI(
    private val displayName: String,
    private val configScanner: () -> Unit = {}
) : Configurable {
    
    protected var panel: JPanel? = null
    protected var tree: Tree? = null
    protected var cardPanel: JPanel? = null
    protected var cards: CardLayout? = null
    protected val formBuilders = mutableMapOf<String, DynamicFormBuilder>()
    ...
    protected open fun createTreePanel(): JPanel {
        ...
        tree = Tree(treeModel).apply {
            ...
            addTreeSelectionListener { event ->
                ...
                node?.userObject?.let { nodeName ->
                    ...
                    // 检查是否存在对应的配置面板
                    if (cardPanel?.componentCount ?: 0 > 0) {
                        // 尝试显示对应的面板
                        try {
    ...
    protected open fun buildPathFromNode(node: DefaultMutableTreeNode): List<String> {
        val path = mutableListOf<String>()
        var currentNode: DefaultMutableTreeNode? = node
        
        // 从当前节点向上遍历到根节点，构建路径
        while (currentNode != null && currentNode.userObject != displayName) {
    ...
    protected open fun initializeTreeAndPanels() {
    ...
    protected open fun buildTreeStructure(
    ...
    protected open fun createConfigPanel(
