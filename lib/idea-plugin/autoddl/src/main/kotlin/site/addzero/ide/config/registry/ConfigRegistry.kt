...
object ConfigRegistry {
     ...
     * 注册配置类
     *
     * @param configClass 带 @Route 注解的配置类
     */
    fun registerConfig(configClass: KClass<*>) {
        val routeAnnotation = configClass.findAnnotation<Route>()
        if (routeAnnotation != null) {
            // 生成唯一标识符
            val id = routeAnnotation.path.joinToString(".") + "." + configClass.simpleName
            ...
            val routeInfo = ConfigRouteInfo(
                path = routeAnnotation.path.toList(),
