...
fun List<LsiClass>.toCompleteSchemaDDL(): String {
  ...
  this.forEach { lsiClass ->
    val generateCreateTable = strategy.generateCreateTable(lsiClass)
    statements.add("-- Table: ${lsiClass.name}")
    statements.add(generateCreateTable)
    statements.add("")
  }
...
/**
 * 生成差量 DDL（包含表、列、索引、外键的完整差量）
 *
 * 此方法会调用 MainLogic 中的 generateDiffDdl() 生成完整的差量 DDL
 *
 * @see site.addzero.util.ddlgenerator.extension.generateDiffDdl
 */
fun List<LsiClass>.toDiffDDL(): String = generateDiffDdl().toSql()
