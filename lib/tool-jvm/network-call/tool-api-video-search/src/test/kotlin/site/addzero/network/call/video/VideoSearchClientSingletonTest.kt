...
class VideoSearchClientSingletonTest {

  @Test
  fun testGetInstance() {
    val instance1 = VideoSearchClient.getInstance()
    val instance2 = VideoSearchClient.getInstance()
  ...
  fun testGetInstanceCaching() {
    ...
    val sources1 = VideoSearchClient.getSearchSources()
    val sources2 = VideoSearchClient.getSearchSources()
    
    assertEquals(sources1, sources2, "单例应该缓存搜索源列表")
    assertTrue(sources1.size == 4, "应该有4个搜索源")
  ...
  fun testGetInstanceConsistency() {
    ...
    val source1 = VideoSearchClient.getSearchSourceByName("豆瓣")
    val source2 = VideoSearchClient.getSearchSourceByName("豆瓣")
    
    assertEquals(source1, source2, "不同实例应该返回相同结果")
    assertEquals("豆瓣", source1?.name)
  ...
  fun testGetInstancePerformance() {
    ...
    repeat(iterations) {
      val instance = VideoSearchClient.getInstance()
      VideoSearchClient.getSearchSources()
    }
