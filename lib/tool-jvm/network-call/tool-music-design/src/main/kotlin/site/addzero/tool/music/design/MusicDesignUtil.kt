 ...
 * 整合歌词搜索和 Suno 音乐生成功能
 * 
 * @param apiToken Suno API Token，如果不传则从环境变量 SUNO_API_TOKEN 读取
 */
class MusicDesignUtil(apiToken: String? = null) {
    
    private val sunoClient: SunoClient = run {
        val token = apiToken ?: System.getenv("SUNO_API_TOKEN")
            ?: throw IllegalStateException("未设置 Suno API Token，请传入 apiToken 参数或设置环境变量 SUNO_API_TOKEN")
        SunoClient(token)
    }
    ...
    fun remixBySongName(
        ...
        // 生成音乐
        return sunoClient.generateMusicCustom(
            lyrics = cleanLyrics,
            title = songName,
            tags = tags,
            model = model
    ...
    fun remixBySongNameAndWait(
        ...
        val taskId = remixBySongName(songName, artistName, tags, model) ?: return null
        return sunoClient.waitForCompletion(taskId, maxWaitTimeSeconds)
    }
    ...
    fun remixByLyricFragmentAndWait(
        ...
        val taskId = remixByLyricFragment(lyricFragment, tags, model) ?: return null
        return sunoClient.waitForCompletion(taskId, maxWaitTimeSeconds)
    }
    ...
    fun batchRemixBySongNamesAndWait(
        ...
        val taskIds = batchRemixBySongNames(songInfos, tags, model)
        return sunoClient.waitForBatchCompletion(taskIds, maxWaitTimeSeconds)
    }
    ...
    fun createMusicWithLyricsAndWait(
        ...
        val taskId = createMusicWithLyrics(lyrics, title, tags, model)
        return sunoClient.waitForCompletion(taskId, maxWaitTimeSeconds)
    }
    ...
    fun fetchTask(taskId: String): SunoTask? {
        return sunoClient.fetchTask(taskId)
    }
    ...
    fun batchFetchTasks(taskIds: List<String>): List<SunoTask> {
        return sunoClient.batchFetchTasks(taskIds)
    }
