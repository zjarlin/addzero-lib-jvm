...
interface CodeGenerator {
     ...
     *
     * Sources of corresponding [KSNode]s which are obtained directly from [KLResolver] need to be specified.
     * Namely, the containing files of those [KSNode]s who are obtained from:
     *   * [KLResolver.getAllFiles]
     *   * [KLResolver.getSymbolsWithAnnotation]
     *   * [KLResolver.getClassDeclarationByName]
     *
     ...
     * where `A` extends `B`. The processor got `A` by [KLResolver.getSymbolsWithAnnotation] and then got `B` by
     ...
     * @return KLOutputStream for writing into files.
     ...
     * @see [CodeGenerator] for more details.
     */
    fun associate(sources: List<KLFile>, packageName: String, fileName: String, extensionName: String = "kt")
     ...
     * @see [CodeGenerator] for more details.
     */
    fun associateByPath(sources: List<KLFile>, path: String, extensionName: String = "kt")
    ...
    val generatedKLFile: Collection<KLFile>
     ...
     *              directly from [KLResolver] are required.
     * @param packageName corresponds to the relative path of the generated file; using either '.'or '/' as separator.
     * @param fileName file name
     * @param extensionName If "kt" or "java", this file will participate in subsequent compilation.
     *                      Otherwise its creation is only considered in incremental processing.
    ...
    fun associateWithFunctions(
        functions: List<KLFunctionDeclaration>,
        packageName: String,
        fileName: String,
        extensionName: String = "kt"
    ) = Unit
    ...
    fun associateWithProperties(
        properties: List<KLPropertyDeclaration>,
        packageName: String,
        fileName: String,
        extensionName: String = "kt"
    ) = Unit
...
class Dependencies private constructor(
    ...
    val originatingFiles: List<KLFile>
) {
     ...
     */
    constructor(aggregating: Boolean, vararg sources: KLFile) : this(false, aggregating, sources.toList())
