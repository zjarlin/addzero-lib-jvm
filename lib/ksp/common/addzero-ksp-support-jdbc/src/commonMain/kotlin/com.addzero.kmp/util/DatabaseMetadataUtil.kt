...
import com.addzero.kmp.entity.ForeignKeyMetadata
import com.addzero.kmp.entity.JdbcColumnMetadata
import com.addzero.kmp.entity.JdbcTableMetadata
import com.addzero.kmp.entity.PrimaryKeyMetadata
...
object DatabaseMetadataUtil {
    ...
    fun getprimaryKeysMetadata(connection: Connection, schema: String): MutableList<PrimaryKeyMetadata> {
        ...
        while (primaryKeys.next()) {
            result.add(
                PrimaryKeyMetadata(
                    tableName = primaryKeys.getString("TABLE_NAME"), columnName = primaryKeys.getString("COLUMN_NAME"), keySeq = primaryKeys.getShort("KEY_SEQ"), pkName = primaryKeys.getString("PK_NAME")
                )
            )
        }
        return result
    ...
    fun getForeignKeysMetadata(schema: String = "public", connection: Connection): List<ForeignKeyMetadata> {
        ...
        while (foreignKeys.next()) {
            result.add(
                ForeignKeyMetadata(
                    pkTableName = foreignKeys.getString("PKTABLE_NAME"), pkColumnName = foreignKeys.getString("PKCOLUMN_NAME"), fkTableName = foreignKeys.getString("FKTABLE_NAME"), fkColumnName = foreignKeys.getString("FKCOLUMN_NAME"), keySeq = foreignKeys.getShort("KEY_SEQ"), fkName = foreignKeys.getString("FK_NAME"), pkName = foreignKeys.getString("PK_NAME")
                )
            )
        }
    ...
    fun getColumnsMetadata(
        ...
        while (columns.next()) {
            ...
            val element = JdbcColumnMetadata(
                tableName = tableName, columnName = columnName, jdbcType = jdbcType, columnType = normalizedType, columnLength = if (normalizedType in listOf("varchar", "char")) columnSize else null, nullable = nullableBool, nullableFlag = nullableFlag, remarks = remarks, defaultValue = defaultValue, isPrimaryKey = false  // 稍后会更新此字段
            )
            result.add(
                element
            )
