...
interface MetadataExtractor<T> {
     ...
     * @return 依赖的源文件列表
     */
    fun getDependencies(resolver: Resolver, metadata: List<T>): List<com.google.devtools.ksp.symbol.KSFile> = emptyList()
}
...
abstract class AbstractMetadataExtractor<T> : MetadataExtractor<T> {
    ...
    override fun getDependencies(resolver: Resolver, metadata: List<T>): List<com.google.devtools.ksp.symbol.KSFile> {
        // 默认返回所有相关文件，子类可以精确指定
        return emptyList()
    }
}
