            ...
            override fun finish() {
                ...
                if (ModbusCodegenMode.CONTRACT in modes) {
                    ...
                    writeArtifacts(
                        ...
                        artifacts = ModbusArtifactRenderer.renderTransportContractArtifacts(ModbusTransportKind.RTU, services.map(CollectedModbusService::model)),
                    )
                }
                ...
                if (ModbusCodegenMode.SERVER in modes) {
                    writeArtifacts(
                        codeGenerator = environment.codeGenerator,
                        dependencies = dependenciesFor(services.flatMap(CollectedModbusService::originatingFiles), aggregating = true),
                        artifacts = ModbusArtifactRenderer.renderServerArtifacts(ModbusTransportKind.RTU, services.map(CollectedModbusService::model)),
                    )
                }
            }
        }
