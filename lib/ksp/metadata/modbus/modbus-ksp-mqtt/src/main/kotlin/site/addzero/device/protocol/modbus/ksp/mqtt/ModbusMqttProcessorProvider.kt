package site.addzero.device.protocol.modbus.ksp.mqtt
 ...
 * Modbus MQTT 代码生成入口。
 */
class ModbusMqttProcessorProvider : SymbolProcessorProvider {
    override fun create(environment: SymbolProcessorEnvironment): SymbolProcessor {
        val transport = ModbusTransportKind.MQTT
        ...
        if (transport !in enabledTransports) {
            environment.logger.logging(
                "Skip Modbus MQTT generation because ${transport.transportId} is not enabled in ${site.addzero.device.protocol.modbus.ksp.core.ModbusKspOptions.TRANSPORTS_OPTION}.",
            )
