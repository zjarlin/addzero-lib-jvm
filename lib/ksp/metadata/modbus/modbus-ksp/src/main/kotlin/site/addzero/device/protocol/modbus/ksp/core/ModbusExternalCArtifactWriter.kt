...
class ModbusExternalCArtifactWriter private constructor(
    ...
    companion object {
        fun from(environment: SymbolProcessorEnvironment): ModbusExternalCArtifactWriter? {
            val config = ModbusConsumerCArtifactsSupport.resolve(environment) ?: return null
            return fromConfig(
                config = config,
        ...
        internal fun fromConfig(
            ...
            logger: KSPLogger,
        ): ModbusExternalCArtifactWriter? {
            val projectDir = config.firmwareProjectDir.absoluteFile
            ...
            val bridgeImplPath = config.bridgeImplTargetPath.ifBlank { "Core/Src/modbus" }
            val markdownOutputPath = config.markdownOutputPath.ifBlank { "Docs/generated/modbus" }
            logger.logging(
