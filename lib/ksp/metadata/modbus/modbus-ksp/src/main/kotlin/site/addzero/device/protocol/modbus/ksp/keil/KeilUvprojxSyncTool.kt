...
class KeilUvprojxSyncTool : ModbusProjectSyncTool {
    ...
    override fun isEnabled(context: ModbusProjectSyncContext): Boolean =
        resolveConfig(context)?.keilUvprojxPath?.isNotBlank() == true

    override fun sync(context: ModbusProjectSyncContext) {
        val config = resolveConfig(context) ?: return
        val uvprojxPath = config.keilUvprojxPath?.trim().orEmpty()
        if (uvprojxPath.isBlank()) {
            return
        }
        val uvprojxFile = resolveConfiguredFile(config, uvprojxPath)
        ...
        val targetName = config.keilTargetName.trim()
        val groupName = config.keilGroupName.trim().ifBlank { DEFAULT_GROUP_NAME }
