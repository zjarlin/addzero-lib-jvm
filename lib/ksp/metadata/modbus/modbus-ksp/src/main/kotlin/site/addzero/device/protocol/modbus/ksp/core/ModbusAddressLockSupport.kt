...
class ModbusAddressLockFile internal constructor(
    ...
    private data class LoadedAddressLockState(
        val metaTransport: String?,
        val entries: Map<ModbusAddressLockKey, ModbusAddressLockEntry>,
    )
    ...
    fun load(
        ...
        loadState(logger)
            .also { state ->
                val lockedTransport = state.metaTransport
                require(
                    lockedTransport == null ||
                        lockedTransport == transport.transportId ||
                        lockedTransport == MULTI_TRANSPORT_ID,
                ) {
                    "地址锁定文件 ${file.absolutePath} 绑定的是 transport=$lockedTransport，当前生成的是 ${transport.transportId}。"
                }
            }.entries
            .filterKeys { key -> key.transport == transport }

    private fun loadState(logger: KSPLogger): LoadedAddressLockState {
        if (!file.exists()) {
            return LoadedAddressLockState(
                metaTransport = null,
                entries = emptyMap(),
            )
        }
        ...
        file.readLines(Charsets.UTF_8).forEachIndexed { index, rawLine ->
            ...
            when {
                ...
                key.startsWith("op.") -> {
                    ...
                    require(keyParts.size == 4 || keyParts.size == 5) {
                    ...
                    val transport =
                        when (keyParts.size) {
                            5 -> ModbusTransportKind.parseConfigured(keyParts[0]).single()
                            4 -> {
                                val legacyTransport = schemaVersion["transport"]
                                require(!legacyTransport.isNullOrBlank()) {
                                    "地址锁定文件 ${file.absolutePath} 第 ${index + 1} 行缺少 transport 维度，且文件头未声明 meta.transport。"
                                }
                                ModbusTransportKind.parseConfigured(legacyTransport).single()
                            }

                            else -> error("unreachable")
                        }
                    val serviceIdIndex = if (keyParts.size == 5) 1 else 0
                    val addressKey =
                        ModbusAddressLockKey(
                            transport = transport,
                            serviceId = keyParts[serviceIdIndex],
                            operationId = keyParts[serviceIdIndex + 1],
                            functionCodeName = keyParts[serviceIdIndex + 2],
                            addressSpace = ModbusAddressSpace.valueOf(keyParts[serviceIdIndex + 3]),
                        )
                    entries[addressKey] =
        ...
        return LoadedAddressLockState(
            metaTransport = schemaVersion["transport"],
            entries = entries,
        )
    }
    ...
    fun write(
        ...
        val previousState = loadState(noOpLogger())
        val mergedEntries = linkedMapOf<ModbusAddressLockKey, ModbusAddressLockEntry>()
                    ...
                    .map { operation ->
                        val key =
                            ModbusAddressLockKey(
                                ...
                                functionCodeName = operation.functionCodeName,
                                addressSpace = operation.addressSpace,
                            )
                        key to
                            ModbusAddressLockEntry(
                                key = key,
                                ...
                                registerSpan = operation.registerSpan,
                            )
                    }
            }.forEach { (key, entry) ->
                mergedEntries[key] = entry
            }
        val distinctTransports = mergedEntries.keys.map(ModbusAddressLockKey::transport).distinct()
        val useTransportScopedKeys = distinctTransports.size > 1
        val metaTransport =
            if (useTransportScopedKeys) {
                MULTI_TRANSPORT_ID
            } else {
                distinctTransports.singleOrNull()?.transportId ?: transport.transportId
            }
        val content =
            buildString {
                ...
                appendLine("meta.transport=$metaTransport")
                mergedEntries.values
                    .sortedWith(
                        compareBy<ModbusAddressLockEntry>({ it.key.transport.transportId }, { it.key.serviceId }, { it.key.operationId }),
                    ).forEach { entry ->
                        val keyParts =
                            buildList {
                                if (useTransportScopedKeys) {
                                    add(entry.key.transport.transportId)
                                }
                                add(entry.key.serviceId)
                                add(entry.key.operationId)
                                add(entry.key.functionCodeName)
                                add(entry.key.addressSpace.name)
                            }
                        val value =
                            listOf(
                                entry.address.toString(),
                                entry.quantity.toString(),
                                entry.registerSpan.toString(),
                            ).joinToString("|")
                        appendLine("op.${keyParts.joinToString("|")}=$value")
                    }
            }
    ...
    companion object {
        private const val MULTI_TRANSPORT_ID = "multi"

        fun from(environment: SymbolProcessorEnvironment): ModbusAddressLockFile? {
            val rawPath = environment.resolveAddressLockPath().orEmpty()
            if (rawPath.isEmpty()) {
                return null
            }
...
object ModbusAddressPlanner {
    fun resolveServices(
        ...
        services.forEach { service ->
            service.operations.forEach { operation ->
                val key = operation.lockKey(serviceId = service.serviceId, transport = service.transport)
                val address =
                    when {
            ...
            service.copy(
                operations =
                    service.operations.map { operation ->
                        operation.copy(
                            address = assignments.getValue(
                                operation.lockKey(serviceId = service.serviceId, transport = service.transport),
                            ),
                        )
                    },
            )
    ...
    private fun ModbusOperationModel.lockKey(
        serviceId: String,
        transport: ModbusTransportKind,
    ): ModbusAddressLockKey =
        ModbusAddressLockKey(
