...
class ModbusAddressLockSupportTest {
    ...
    @Test
    fun persistMergesDifferentTransportsIntoOneLockFile() {
        val tempDir = createTempDirectory("modbus-address-lock-multi").toFile()
        try {
            val lockPath = tempDir.resolve("device.addresses.lock")
            val rtuServices = listOf(semanticCoilService())
            val tcpServices =
                listOf(
                    semanticCoilService().copy(
                        ...
                        operations =
                            semanticCoilService()
                                .operations
                                .map { operation -> operation.copy(requestClassName = "DeviceServiceTcpGetDeviceInfoRequest") },
                    ),
                )

            ModbusAddressPlanner.persist(rtuServices, ModbusAddressLockFile(lockPath))
            ModbusAddressPlanner.persist(tcpServices, ModbusAddressLockFile(lockPath))

            val text = lockPath.readText()
            assertTrue(text.contains("meta.transport=multi"))
            assertTrue(text.contains("op.rtu|device|get-device-info|READ_COILS|COIL_READ=0|24|24"))
            assertTrue(text.contains("op.tcp|device|get-device-info|READ_COILS|COIL_READ=0|24|24"))

            val tcpEntries = ModbusAddressLockFile(lockPath).load(ModbusTransportKind.TCP, noOpLogger())
            assertEquals(1, tcpEntries.size)
            assertTrue(tcpEntries.keys.all { key -> key.transport == ModbusTransportKind.TCP })
        } finally {
            tempDir.deleteRecursively()
        }
    }
