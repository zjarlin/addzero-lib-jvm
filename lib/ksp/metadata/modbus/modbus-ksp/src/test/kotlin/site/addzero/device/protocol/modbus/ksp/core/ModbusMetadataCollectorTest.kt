        ...
        val services =
            ModbusMetadataCollector.collect(
                environment =
                    testEnvironment(
                        mapOf(
                            "addzero.modbus.metadata.providers" to "interfaces",
                            "addzero.modbus.database.driverClass" to "missing.Driver",
                            "addzero.modbus.database.jdbcUrl" to "jdbc:invalid",
                            "addzero.modbus.database.query" to "select payload from modbus_metadata",
                        ),
                    ),
            ...
            assertFailsWith<IllegalStateException> {
                ModbusMetadataCollector.collect(
                    environment =
                        testEnvironment(
                            mapOf(
                                "addzero.modbus.metadata.providers" to "interfaces,unknown",
                            ),
                        ),
                    resolver = emptyResolver(),
                    transport = ModbusTransportKind.RTU,
