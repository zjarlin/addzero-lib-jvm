...
- 业务模块要编译 `modbus-ksp` 生成出来的 Kotlin 源码。
...
- 你可以先把 `modbus-ksp` 在 MQTT transport 下生成出来的代码编译通过。
...
这些事情要交给 `modbus-ksp` 主处理器；如果还需要外部 C 工程、Keil、CubeMX 的落盘与同步，则由消费方额外挂到 `ksp(...)` classpath 上的 SPI provider 决定。
