...
object CurlUtil {
    ...
    fun generateCurlCommand(request: HttpServletRequest, args: Array<Any>): String? {
...
//        val s2 = StrUtil.addSuffixIfNot(s11, fix)
        ...
        val fix = System.lineSeparator()
        val s11 = if (s.startsWith(fix)) s else "$fix$s"
        val s2 = if (s11.endsWith(fix)) s11 else "$s11$fix"
        return s2
    }
    ...
    private fun needHeader(headerName: String): Boolean {
        ...
        val b = list.stream().anyMatch { e ->
            e?.contains(headerName, ignoreCase = true) == true
        }
        return b
    }
