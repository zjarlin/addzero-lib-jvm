...
插件会自动应用到所有子项目 latestversion 可能会有延迟,so nextVersion亦有延迟,如果你发版太快,会出现不自洽的情况!!!  那么这时候应当手动控制版本(
那么nextVersion读取程序中现有的版本肯定不会有延迟问题)
...
```kotlin
//想获取某个groupId对应的最新版本号
//+匹配最新也是这个原理,但是maven的最新逻辑好像有bug上述已经说明
引入 implementation ("site.addzero.tool-api-maven") version "+"
例如 
MavenCentralSearchUtil.getLatestVersionByGroupId("org.babyfish.jimmer")
