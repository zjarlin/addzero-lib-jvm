            ...
            if (tableDictFields.isNotEmpty()) {
                ...
                tableDictFields.forEach { dictField ->
                    dictField.dictConfigs.filter { !it.isSystemDict }.forEach { config ->
                        ...
                        appendLine("            String keys = String.join(\",\", ${mapKey}_keys);")
                        appendLine("            List<TableDictModelResult> results = transApi.translateTableBatchCode2name(")
                        appendLine("                \"${config.tableName}\", \"${config.nameColumn}\", \"${config.codeColumn}\", keys);")
                        appendLine("            ${mapKey}_map = results.stream()")
